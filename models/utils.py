...
class PatchEmbed(nn.Module): ## 输入 B C H W  输出 B H W C
...
class Downsample(nn.Module): #输入：B H W C  输出: B H W C
    """ Image to Patch Embedding
    """
    ...
    def __init__(self, in_embed_dim, out_embed_dim, patch_size, overlap=False):
        ...
        if overlap:
            ...
            self.proj = nn.Conv2d(in_embed_dim, out_embed_dim, kernel_size=patch_size, stride=patch_size) # [i+2*p-k]/s+1
