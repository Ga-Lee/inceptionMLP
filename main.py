...
parser.add_argument('--model', default='inception_s', type=str, metavar='MODEL',
...
parser.add_argument( '--validation-batch-size-multiplier', type=int, default=1, metavar='N',
...
def main():
    ...
    if args.distributed:
        # print("args.distributed"+str(args.rank)+"-yes")
        logging.info('Training in distributed mode with multiple processes, 1 GPU per process. Process %d, total %d.'
                     % (args.rank, args.world_size))
    else:
        # print("args.distributed"+str(args.rank)+"-no")
    ...
    # print(f'args.num_gpu: {args.num_gpu}');exit()
    if args.num_gpu > 1:
        if use_amp == 'apex':
            logging.warning(
    ...
    if local_rank == 0:
        ...
        yaml_path = os.path.join(output_dir, 'args.yaml')
        print('yaml_path:',yaml_path)
        with open(yaml_path, 'w') as f:
            f.write(args_text)
        ## 启动wandb
        run_name = '_'.join([str(args.lr), str(args.batch_size) ])
        wandb.init(project = exp_name, name = run_name) 
        wandb.watch(model, log="all") 
        logging.info(f"Start training for {num_epochs} epochs from epoch {start_epoch}")
    ...
    try:  # train the model
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_epoch(
                ...
                amp_autocast=amp_autocast, loss_scaler=loss_scaler, model_ema=model_ema, mixup_fn=mixup_fn, local_rank=local_rank)
            ...
            eval_metrics = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, local_rank = local_rank)

            print("eval")
            
            # print(f"Accuracy of the network: {eval_metrics['top1']:.1f}%")

            if model_ema is not None and not args.model_ema_force_cpu:
                if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                    distribute_bn(model_ema, args.world_size, args.dist_bn == 'reduce')
                ema_eval_metrics = validate(
                    model_ema.ema, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, log_suffix=' (EMA)',local_rank=local_rank)
                eval_metrics = ema_eval_metrics ## 保存的都是ema 
            ...
            if local_rank == 0:
                ...
                'acc5': eval_metrics['top5'],
                'eval_loss': eval_metrics['loss']
            })
                logging.warning(f"Epoch {epoch}: Train loss: {train_metrics['loss']:.4g} Eval_loss: {eval_metrics['loss']} lr: {train_metrics['lr']} acc1: {train_metrics['loss']:.4g} acc5: {eval_metrics['top5']}" )   
...
def train_epoch(
        ...
        loss_scaler=None, model_ema=None, mixup_fn=None,local_rank=0):
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='',local_rank=0):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    # if args.local_rank == 0:
    if local_rank == 0:
        logging.info('epoch eval Acc@1: {top1.avg:>7.4f}   ''Acc@5:{top5.avg:>7.4f}'.format(top1=top1_m, top5=top5_m))
